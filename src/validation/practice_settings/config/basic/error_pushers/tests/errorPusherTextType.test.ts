...
import errorPusherTextType from '../errorPusherTextType';
...
describe('errorPusherTextType', () => {
  const validValues = ['quote', 'medley', 'custom'];

  describe('should', () => {
    describe('push correct error when', () => {
      function assertPushesError(value: any) {
        test(`value=${JSON.stringify(value)}`, () => {
          errorPusherTextType(value, 'test', errors);
          expect(errors).toHaveLength(1);
          expect(errors[0]).toMatch('test.basic.config.textType');
