...
import errorPusherCaretStyle from '../errorPusherCaretStyle';
...
describe('errorPusherCaretStyle', () => {
  const validValues = ['bar', 'block', 'underline', 'outline'];

  describe('should', () => {
    describe('push correct error when', () => {
      function assertPushesError(value: any) {
        test(`value=${JSON.stringify(value)}`, () => {
          errorPusherCaretStyle(value, 'test', errors);
          expect(errors).toHaveLength(1);
          expect(errors[0]).toMatch('test.basic.config.caretStyle');
